...
import performing from "../../assets/venues/performing.webp";

const VenuePerforming = () => {
  return (
    <section className="py-20 bg-bg-soft overflow-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-4 sm:gap-8 items-center mb-16">
        ...
        >
          ...
          >
            A 500-seat premium theater built for performance, storytelling, and
            broadcast-ready experiences — combining acoustically tuned design,
            immersive staging, and seamless production infrastructure for live,
            corporate, and branded events.
          </motion.p>

          {/* STATS */}
          <div className="grid grid-cols-3 gap-px mt-8 bg-[var(--color-border)]">
            {performingStats.map((item, i) => (
              <motion.div
                key={item.label}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        <p className="text-text-muted text-sm mt-3 max-w-md mx-auto leading-relaxed">
          Limited availability for performances, corporate, and private events.
        </p>
...
export default VenuePerforming;